...
- For Brief mode, write a longer narrative (300-400 words) covering all threat categories.
  ...
  try {
    ...
    if (context) {
      if (context.disasters && context.disasters.length > 0) {
        ...
        dataContext += JSON.stringify(context.disasters.slice(0, 50), null, 0);
      }
      if (context.alerts && context.alerts.length > 0) {
        dataContext += `\n\nACTIVE NWS WEATHER ALERTS (${context.alerts.length} total):\n`;
        dataContext += JSON.stringify(context.alerts.slice(0, 50), null, 0);
      }
      if (context.region) {
